...
sub handler {
	...
	{			# Save routine.. pull out the form and save them to disk
		...
		# sugarmipsize (Henk, 2026-09-03) - moved from a global-only setting to per-player, under
		# SugarCube Mix Mode. Now used by both Standard MusicIP (buildMIPReq) and FreeStyle mode
		# (FSgetRealRandomSubset* in Breakout.pm).
		...
		# DropArtists/getRandom/DropGenreAndXMas/getRealRandom. CHANGED 2026-09-03 (Henk): was
		# three separate exact-name fields per type (scblockartist_always/alwaystwo/alwaysthree),
		# now one comma-separated field, so the list isn't capped at three - matching saysaar's
		# SC-EXTMIP build. OLD PREFS are simply abandoned, not migrated - re-enter them here as
		# one list.
		my $scblockartist_list = $params->{'scblockartist_list'};
		$prefs->client($client)->set('scblockartist_list', "$scblockartist_list");

		# Artist Weighting - CHANGED 2026-09-03 (Henk): was three individually-weighted name
		# slots per side (Prefer/Less), now one comma-list per side sharing ONE weight (1-5),
		# matching saysaar's SC-EXTMIP build.
		...
		# MEGA SAVE START
			my $sc_ownid = Slim::Player::Client::id($client);
			my @sc_synced_players = ();
			foreach my $player (Slim::Player::Client::clients()) {
				my $sc_pid = Slim::Player::Client::id($player);
				next if ($sc_pid eq $sc_ownid);
				next unless ($params->{"sc_sync_player_$sc_pid"});
				push @sc_synced_players, $sc_pid;
				...
				$prefs->client($player)->set('sugarmipsize', "$sugarmipsize");
				...
				$prefs->client($player)->set('sugarcube_filetype',"$sugarcube_filetype");		# FreeStyle Mode (mp3/flac)

				$prefs->client($player)->set('sugarcube_mode', "$sugarcube_mode");		# Standard MusicIP or Freestyle mode

				$prefs->client($player)->set('sugarcube_year_on_off',"$sugarcube_year_on_off");	# 0=any 1=strict 2=allow empty

				$prefs->client($player)->set('sugarcube_startyear',"$sugarcube_startyear");	# FS Start Year
				$prefs->client($player)->set('sugarcube_endyear',"$sugarcube_endyear");		# FS End Year

				$prefs->client($player)->set('sugarcube_fs_length',"$sugarcube_fs_length");		# FS Length
				...
				$prefs->client($player)->set('sugarcube_fade_on_off', "$sugarcube_fade_on_off");		# Fade or not fade
				$prefs->client($player)->set('sugarcube_fade_time', "$sugarcube_fade_time");		# Fade timeline

				$prefs->client($player)->set('sugarcube_dupper', "$sugarcube_dupper");
			...
			$prefs->client($client)->set('sugarcube_megasaver_players', join(',', @sc_synced_players));
		}
		# MEGA SAVE END
...
#
	...
	$params->{'prefs'}->{'sugarmipsize'}  = $prefs->client($client)->get('sugarmipsize');
	...
	my $sugarcube_megasaver_players = $prefs->client($client)->get('sugarcube_megasaver_players');
	my %sc_sync_selected = ();
	if ($sugarcube_megasaver_players) {
		foreach my $sc_pid ( split(/,/, $sugarcube_megasaver_players) ) {
			$sc_sync_selected{$sc_pid} = 1;
		}
	}
	$params->{'prefs'}->{'sc_sync_selected'} = \%sc_sync_selected;
...
#####
# PLAYERS (for the "Sync Settings Across CHOSEN Players" checklist)
#
sub getPlayerList {
	my $client = shift;
	my %playerHash = ();
