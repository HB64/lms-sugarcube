...
use URI::Escape;
# MIP Export (Henk, 2026-09-02) - ported from the saysaar/SC-EXTMIP build/HB64 fork. None of these
# three were used anywhere else in this file before this.
use LWP::UserAgent;
...
# MIP Export state - see ExportStatsToMIP. sc_mipexport_inprogress/sc_mipexport_result are real
# prefs (not just here) so the settings page can poll them via the generic 'pref' JSON-RPC command.
...
"SugarCube MIP URL not Set; $miphosturl for this client, default set to localhost\n"
    ...
    ###
    # MIP EXPORT (Henk, 2026-09-02) - ported from the saysaar/SC-EXTMIP build and HB64 fork. Reuses
    # sugarport/miphosturl and this build's own DPC prefs (nasconvertpath/localmediapath via
    # scPathPair() - see the note near that sub) rather than separate connection/path prefs.
    # useapcvalues is respected the same way the rest of this file already does.
    ###
    ...
    my $sc_mipexport_time = $prefs->get('sc_mipexport_time');
    if ( !defined $sc_mipexport_time || $sc_mipexport_time eq '' ) {
        $sc_mipexport_time = '04:17';
        $prefs->set( 'sc_mipexport_time', $sc_mipexport_time );
        $log->debug("SC MIP Export time not set; default set to 04:17\n");
    }
    my $sc_mipexport_rating_threshold = $prefs->get('sc_mipexport_rating_threshold');
    if ( !defined $sc_mipexport_rating_threshold ) {
        $prefs->set( 'sc_mipexport_rating_threshold', 0 );
        $log->debug("SC MIP Export rating threshold not set; default set to 0 (off)\n");
    }

    # A library rescan changes nothing for existing ratings/playcounts by itself, but a NEW track
    # has nothing in MIP's cache yet - export once, ten seconds after the scan settles.
    Slim::Control::Request::subscribe( \&scMIPExportPostScanTimer, [ [ 'rescan' ], [ 'done' ] ] );

    # sugarlviconsize's own settings.html field was removed 2026-09-03 (Henk) - no longer
    # user-adjustable, so whatever value happened to be stored back then (50) is what every
    # cover using this pref has been stuck at since, making them look tiny. Henk's request
    # 2026-09-04: since there's no UI to change it anymore, this IS now the fixed default -
    # forced to 100 every startup instead of only filled in when unset.
    ...
    # sugarmipsize moved to a per-player setting 2026-09-03 (Henk) - see buildMIPReq's own
    # lazy-default (matching the sugarcube_style/sugarcube_variety pattern there) and
    # FSgetRealRandomSubset*'s in Breakout.pm. Global default no longer set here.
    ...
    # Live View's two buttons under Currently Playing / Coming Up Next, Henk's request
    # 2026-09-04 - both reached as a plain slim.request from the page itself (same JSON-RPC
    # pattern as scPlayerCmd's Previous/Play-Pause/Next), so Live View never has to navigate
    # away and back for either. See scStartMix/scReplaceNext below.
    Slim::Control::Request::addDispatch( [ 'sugarcube', 'startmix' ],
        [ 1, 0, 0, \&scStartMix ] );
    Slim::Control::Request::addDispatch( [ 'sugarcube', 'replacenext' ],
        [ 1, 0, 0, \&scReplaceNext ] );

    # "Play Next" icon on each MusicIP Track Recommendations row, Henk's request 2026-09-05.
    # Deliberately NOT a plain playlistcontrol cmd:insert straight from the page (that was the
    # first cut) - Henk pointed out it grows the queue by one every click, which fights his own
    # habit of only ever having "now playing + 1 next" and letting SugarCube top the queue back
    # up only once it runs out. Ported instead from the saysaar build's Live View "Use as Next
    # Track" (scReplaceSelection): delete the current "next" track, then insert this one, so the
    # queue length stays put - and refuse outright (no delete, no insert) when more than one
    # track is already queued ahead, since that means a fired SC Batch or a queue Henk built by
    # hand, not a queue SugarCube can safely disturb. See scPlayNextTrack below, and the matching
    # $scCanReplace check in Breakout.pm's StatsPuller that hides the icon in that same case.
    Slim::Control::Request::addDispatch( [ 'sugarcube', 'playnext', '_trackid' ],
        [ 1, 0, 0, \&scPlayNextTrack ] );
...
sub postinitPlugin {
    ...
    # MIP Export - arm the scheduler once library scanning has settled. scMIPExportScheduler
    # re-arms itself every 30 minutes from here on.
...
sub jiveSugarCubeMenu {
    ...
    my @menuItems = (
...

        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_volume_flag') || 0 ) +
              1,
            actions => {
                do => {
                    choices => [
        ...
        },
        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_sleep') || 0 ) + 1,
            actions => {
                do => {
                    choices => [
                        {
        ...
        },
        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_upnext') || 0 ) + 1,
            actions => {
                do => {
                    choices => [
                        {
...

        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_shuffle') || 0 ) + 1,
            actions => {
                do => {
                    choices => [
                        {
        ...
        },
        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_mix_type') || 0 ) + 1,
...

        {
            ...
            selectedIndex =>
              ( $prefs->client($client)->get('sugarcube_album_song') || 0 ) +
              1,
...
sub toggle_state {
    ...
    if ( ( $prefs->client($client)->get('sugarcube_status') || 0 ) == 1 )
    {    # Enabled
...
###
# scReplaceNext - Live View's "Replace Track" button (under Coming Up Next), Henk's request
# 2026-09-04.
#
# Same SC/MIP replace-next logic Quick Settings' own replace button already uses
# (SugarCubeReplaceNext, previously only reachable via the forcereplace=1 HTTP param, which
# means a full page navigation), but reached as a plain slim.request instead - same JSON-RPC
# shape as scStartMix above, so Live View never has to navigate away and back for it.
###
sub scReplaceNext {
    my $request = shift;
    my $client  = $request->client();
...
# scPlayNextTrack - the "Play Next" icon on each MusicIP Track Recommendations row, Henk's
# request 2026-09-05. Ported from the saysaar build's scReplaceSelection (Live View's "Use as
# Next Track"): swaps the chosen track into the next-to-play slot instead of appending it, so
# repeated clicks don't grow the queue. Guard is identical to that build's - SugarCube only ever
# disturbs a track it queued itself, which continuous play keeps at exactly one ahead of the one
# playing now (CheckPosition == 2). More than that is a fired SC Batch or a queue built by hand,
# and this refuses outright rather than deleting one track out of it. Breakout.pm's StatsPuller
# hides the icon for the same reason before a click is even possible - this is the belt-and-
# braces check for the few seconds between drawing that page and clicking (a batch firing in
# between, say).
sub scPlayNextTrack {
    my $request = shift;
    my $client  = $request->client();
    ...
    return unless defined $trackid && $trackid =~ /^\d+$/;

    my $ahead = Plugins::SugarCube::Breakout::CheckPosition($client);
    if ( $ahead > 2 ) {
        $log->info(
"Play Next ignored - $ahead tracks queued, SugarCube only replaces a queue of its own making\n"
        );
        $request->setStatusDone();
        return;
    }
    ...
    unless ( blessed($track) && $track->url ) {
        $log->warn("Play Next - no track found for id $trackid\n");
        $request->setStatusDone();
        return;
    }
    ...
    addtrack( $client, $track->url, 'insert' );
...
###
    ...
    if ( length($song) == 0 ) {
        ...
        if ( $song eq 'FAILED' ) {
            ...
            $song = Plugins::SugarCube::Breakout::getRealRandom($client);
...
###
# Restore MIP's original ranking before the results get saved to
# WorkingSet. The "WHERE tracks.url = 'a' OR tracks.url = 'b' OR ..."
# metadata queries below have no ORDER BY, so SQLite hands rows back in
# its own internal order, not the order MIP ranked them in - silently
# discarding MIP's acoustic/filter/recipe ranking before SC ever picks a
# track. Re-sort the fetched 11-field-per-track chunks to match the
# position each track's URL had in @unique (MIP's own returned order).
###
sub reorderByMIPRank {
    my ( $unique_ref, $quickone_ref ) = @_;
    my %rank;
    my $i = 0;
    foreach my $u (@$unique_ref) {
        $rank{$u} = $i unless exists $rank{$u};    # keep first occurrence
        $i++;
    }
    my @chunks;
    for ( my $j = 0 ; $j < scalar(@$quickone_ref) ; $j += 11 ) {
        push @chunks, [ @{$quickone_ref}[ $j .. $j + 10 ] ];
    }
    @chunks = sort {
        ( exists $rank{ $a->[0] } ? $rank{ $a->[0] } : 999999 )
          <=> ( exists $rank{ $b->[0] } ? $rank{ $b->[0] } : 999999 )
    } @chunks;
    return map { @$_ } @chunks;
}
...
sub gotMIP {
    ...
    my $changeindex = 0;
    # Read once for the whole response rather than per line. Empty list (both undef) means no
    # conversion needed - matches SC-EXTMIP's scPathPair() exactly, replacing the old separate
    # sugardpc on/off toggle (removed 2026-09-03, Henk - conversion is now implicit: filled-in
    # MusicIP path = convert, empty = don't).
    ...
    foreach (@miparray) {
        ...
        if ( defined $lmspath ) {                 # MusicIP path -> LMS path
            ...
            # simplified to one pair, matching the HB64 fork. Having two pairs only caused
            # confusion. nasconvertpath_2/localmediapath_2 stay orphaned in the prefs file for
            # anyone upgrading from an older build.

            # The substitution above only swaps the matched NAS PREFIX
...
"SELECT tracks.url, tracks.title, albums.title, genres.name, contributors.name, $table.playCount, tracks_persistent.rating, $table.lastPlayed, tracks.coverid, tracks.album, tracks.id FROM contributors, tracks INNER JOIN genre_track ON (genre_track.track = tracks.id) INNER JOIN tracks_persistent ON (tracks.urlmd5 = tracks_persistent.urlmd5)";
        ...
        @quickone = reorderByMIPRank( \@unique, \@quickone );
...
sub randompuller {
    ...
    # (genre-blind) instead.
    my $scblockgenre_list_raw =
      $prefs->client($client)->get('scblockgenre_list') || '';
    my @scblockgenres =
      grep { length($_) }
      map  { s/^\s+|\s+$//g; $_ }
      split( /,/, $scblockgenre_list_raw );
    if (   length($NEWSCgenre)
        && ( grep { $_ eq $NEWSCgenre } @scblockgenres ) )
    {
        $log->debug(
...
sub SugarPlayerCheck {
    ...
    # Henk, 2026-09-04: was +20. That margin dates from when the plugin was built and slow
    # hardware was often the bottleneck reading Lyrion's own database - not a common case
    # anymore, so the recurring re-check interval is brought down to near-immediate.
    my $timer =
...
sub buildMIPReq {
    ...
    my $sugarmipsize  = $prefs->client($client)->get('sugarmipsize');
...
#      $log->debug("No MIP Variety Setting - Set for this client, default set to 0.\n" );
    ...
    if ( !defined $sugarmipsize || $sugarmipsize eq '' ) {
        $sugarmipsize = 10;
        $prefs->client($client)->set( 'sugarmipsize', $sugarmipsize );

        #      $log->debug("No MIP Size Setting - Set for this client, default set to 10.\n" );
    }
    ...
    # Matches SC-EXTMIP's scPathPair() exactly, replacing the old separate sugardpc on/off toggle
    # (removed 2026-09-03, Henk - conversion is now implicit: filled-in MusicIP path = convert,
    # empty = don't).
    my ( $lmspath, $mippath ) = scPathPair();
    ...
    if ( defined $lmspath ) {
        ...
        # Second path pair (nasconvertpath_2/localmediapath_2) removed 2026-09-02 (Henk) - DPC
        # simplified to one pair, matching the HB64 fork. Having two pairs only caused confusion.
        # nasconvertpath_2/localmediapath_2 stay orphaned in the prefs file for anyone upgrading
        # from an older build.
...
"\nKeeping music playing requesting (from LMS db) a Random Track matching the Current Playing Tracks Genre\n"
    ...
    if ( $song eq 'FAILED' ) {
        ...
        my $track = Plugins::SugarCube::Breakout::getRealRandom($client);
...
# scPathPair - the ONE place that decides whether paths need converting, and what to convert.
# Replaces the old separate sugardpc on/off toggle (removed 2026-09-03, Henk, to match
# SC-EXTMIP exactly): conversion is now implicit - a filled-in "Music Path in MusicIP" means
# convert, an empty one means LMS and MusicIP already point at the same files. Returns
# (lmspath, mippath), or an empty list (both undef) when there's nothing to convert.
sub scPathPair {
...
###
# MIP EXPORT (Henk, 2026-09-02) - ported from the saysaar/SC-EXTMIP build (built and debugged over
# several rounds, including fixes from guptaas' review of AF-1's original ExportToMusicIP plugin)
# and from the HB64 fork's own port of the same feature. Adapted here for this build's own
# conventions:
#  - useapcvalues (this build's own APC on/off toggle) is honored.
#  - Path conversion uses this build's own single-pair DPC (nasconvertpath/localmediapath via
#    scPathPair() - simplified from two pairs 2026-09-02, sugardpc on/off toggle removed
#    2026-09-03, see scPathPair() below), mirroring buildMIPReq's own substitution above,
#    including its use of Slim::Utils::Unicode::utf8decode_locale before escaping.
#  - No separate MIP host/port/timeout prefs - reuses sugarport/miphosturl, this plugin's own.
#  - No extension-replace option (as in HB64 - can be added later if it turns out to matter).
#  - Timeout fixed at 15s, not a setting.
###

# Re-arms itself every 30 minutes. Fires the export once per calendar day, no earlier than the
# configured time, no later than 30 minutes after it.
sub scMIPExportScheduler {
    Slim::Utils::Timers::killTimers( undef, \&scMIPExportScheduler );

    return unless $prefs->get('sc_mipexport_enabled');
    ...
    my $exporttime = $prefs->get('sc_mipexport_time') || '04:17';
...
# each call just re-arms the same ten-second timer, so only the last one actually fires.
...
# The main job. Runs as an LMS scheduler task (main::idleStreams between tracks), not a plain sub
# call, so a large library does not stall playback.
sub ExportStatsToMIP {
    ...
    # Same useapcvalues-gated $apc_enabled branch the rest of this file already uses (see the
    # Live View/history query above) - keeps the exported stats consistent with whatever SugarCube
    # itself is reading for its own mixing.
    my $table = ( $apc_enabled && $prefs->get('useapcvalues') ) ? 'alternativeplaycount' : 'tracks_persistent';

    my $query = "SELECT tracks.url, tracks_persistent.rating, $table.playCount, $table.lastPlayed"
        . " FROM tracks INNER JOIN tracks_persistent ON (tracks.urlmd5 = tracks_persistent.urlmd5)";
    $query .= " LEFT JOIN alternativeplaycount ON tracks.urlmd5 = alternativeplaycount.urlmd5" if ( $apc_enabled && $prefs->get('useapcvalues') );
    $query .= " WHERE ifnull(tracks.remote,0) = 0 AND (tracks_persistent.rating > 0 OR ifnull($table.playCount,0) > 0)";
...
# loop checks - the current track still finishes.
...
sub scMIPExportTrack {
    ...
    # ALWAYS SENT, even when rating is 0/unset - a track reaching here already matched the SQL's
    # rating>0 OR playcount>0 test, so rating 0 here means either unrated-but-played (fine, MIP
    # setRating=0 is a no-op there) or WAS rated and has since been demoted/cleared in LMS, which
    # should clear MusicIP's copy too rather than leave it at whatever it was set to previously.
...
# LMS 0-100 -> MIP 0-5. sc_mipexport_rating_threshold is 0-4 "stars" (0 = disabled, the plain
# bucket below). Set to N, any LMS rating below N stars exports as MIP 0 (unrated), and everything
# from N stars upward is stretched to cover MIP's full 1-5 range. The ten-point half-star buffer on
# the cutoff (not on the stretch anchor) protects a track sitting right at the threshold from
# dropping out of the export if APC's automatic rating adjustment nudges it down by a fraction.
sub scMIPConvertRating {
    my $rating100 = shift;
    my $threshold = $prefs->get('sc_mipexport_rating_threshold') || 0;

    unless ( $threshold > 0 && $threshold < 5 ) {
        return 0 if ( !$rating100 || $rating100 < 10 );
        return 1 if $rating100 < 30;
        return 2 if $rating100 < 50;
        return 3 if $rating100 < 70;
        return 4 if $rating100 < 90;
        return 5;
    }

    my $lmsmin = $threshold * 20;
    my $lmsmin_cutoff = $lmsmin - 10;
    $lmsmin_cutoff = 0 if $lmsmin_cutoff < 0;
    return 0 if ( !$rating100 || $rating100 < $lmsmin_cutoff );

    my $lmsrange = 100 - $lmsmin;
    return 1 if $rating100 < $lmsmin; # in the half-star buffer - same MIP 1 the threshold itself gives
    return 1 + int( ( ( $rating100 - $lmsmin ) / $lmsrange ) * 4 + 0.5 );
}

# tracks.url (a file:// URL) -> whatever path MIP itself expects. Mirrors buildMIPReq's own
# tracktitle handling above almost exactly - this build's single-pair DPC (nasconvertpath/
# localmediapath via scPathPair(), replacing the old sugardpc on/off toggle - removed
# 2026-09-03, Henk), including its utf8decode_locale call, then the same OS-branching escape
# logic (both branches now escape - buildMIPReq's own Wintel-LMS/Linux-MIP branch skips
# escaping, which looks like an oversight there; the export needs a MIP-safe URL either way,
# so both branches escape here, matching the SC-EXTMIP/HB64 builds this was ported from and
# proven there over several rounds of debugging).
sub scMIPExportPath {
    my $url = shift;
    ...
    ###
    # EXTENSION REPLACE - not Henk's own setup (his LMS and MIP point at the same files), but
    # kept in for other users: some setups have MIP indexed against a differently-encoded mirror
    # of the library (e.g. LMS plays FLAC, MIP was pointed at an MP3 copy of the same tracks) -
    # without this, the filename sent to MIP carries the wrong extension and the
    # rating/playcount/lastplayed push silently fails to match a track. Ported from the original
    # plugin's getMusicIpURL via SC-EXTMIP, one pref instead of a per-setup guess.
    ###
    ...
    if ( defined $lmspath ) {
        ...
        my $localmediapath = $lmspath;
        $localmediapath = quotemeta $localmediapath;
        $path =~ s/$localmediapath/$nasconvertpath/i;
        $path =~ s/\//\\/g if $nasconvertpath =~ /\\/;

        my $findos = index( $path, "/", 0 );
        if ( $findos != 0 ) {
            # Linux LMS, Wintel MIP.
        ...
        # No MusicIP path set - LMS and MIP point at the same files.
        $path =~ s/\x{FFFD}/%96/g;
        $path = Slim::Utils::Unicode::utf8decode_locale($path);
        $path = escape($path);
        $path =~ s/%2596/%96/g;
...
sub commandCallback {
    ...
    if ( $request->isCommand( [ ['play'] ] ) ) {
        ...
        # Henk, 2026-09-04: was +10. Same old hardware-margin reasoning as the recurring
        # timer inside SugarPlayerCheck itself - brought down to near-immediate.
        Slim::Utils::Timers::setTimer( $client, Time::HiRes::time() + 1,
            \&SugarPlayerCheck );
    ...
    if ( $request->isCommand( [ ['playlist'], ['newsong'] ] ) ) {
        ...
        Slim::Utils::Timers::setTimer( $client,
            ...
            \&SugarDelay );
        # Henk, 2026-09-04: was +15. Same old hardware-margin reasoning as the recurring
        # timer inside SugarPlayerCheck itself - brought down to near-immediate.
        Slim::Utils::Timers::setTimer( $client, Time::HiRes::time() + 1,
            \&SugarPlayerCheck );
...
sub slideVolume {
    ...
    my $sugarcube_reducevolume =
      $prefs->client($client)->get('sugarcube_reducevolume') || 0;
    my $sugarcube_volumetimefrom =
      $prefs->client($client)->get('sugarcube_volumetimefrom') || 0;
    my $sugarcube_volumetimeto =
      $prefs->client($client)->get('sugarcube_volumetimeto') || 0;
    ...
    if ( $sugarcube_sleep == 1 ) {
        ...
        if ( $sleeper == 0 ) {
            ...
            my $sugarcube_sleepfrom =
              $prefs->client($client)->get('sugarcube_sleepfrom') || 0;
            my $sugarcube_sleepto =
              $prefs->client($client)->get('sugarcube_sleepto') || 0;
            my $sugarcube_sleepduration =
              $prefs->client($client)->get('sugarcube_sleepduration') || 0;
...
###
# scStartMix - Live View's "Start New Mix" button, Henk's request 2026-09-04.
#
# Does exactly what clicking Auto Mix does - clears the queue and asks MusicIP for a fresh mix,
# with no seed - by calling the same AutoStartMix that handleWebQP calls. Reached as a plain
# slim.request from the page itself, so Live View never has to navigate to quickplay.html and
# back just to restart the mix. AutoStartMix already guards on sugarcube_status (refuses on a
# disabled player) and shows its own on-player message, so there is nothing left for this
# wrapper to do beyond finding the client and calling it.
###
sub scStartMix {
    my $request = shift;
    my $client  = $request->client();
...
sub handleWebQP {
    ...
    if ($client) {
        ...
        ###
        # SHOW THE SPLASH, THEN JUMP TO LIVE VIEW. Henk's request, 2026-09-04 (ported from
        # saysaar's SC-EXTMIP build, originally 2026-08-24).
        #
        # This handler used to also handle 'forcereplacekickoff'/'forcereplace' for the "New
        # Track"/"Replace Next Track" buttons that used to live on quickplay.html itself. Those
        # buttons and the live playing/up-next display are gone from quickplay.html now - it is
        # just a splash that jumps to Live View a few seconds after landing (see quickplay.html's
        # own script). Equivalent controls on Live View itself are a separate, not yet ported,
        # piece of work.
        #
        # sclvpath is the one thing quickplay.html cannot know on its own - which page "Live
        # View" actually is - so it is handed across rather than hard-coded twice in two files.
        ###
        $params->{sclvpath} = $htmlTemplateLV;
...
sub handleWebQuickSettings {
    ...
    if ($client) {
        ...
        if ( defined( $params->{'scblockgenre_list'} ) ) {
            my $oldval = $prefs->client($client)->get('scblockgenre_list');
            if ( !defined($oldval)
                || $oldval ne $params->{'scblockgenre_list'} )
            {
                $replace = 1;
            }
            $prefs->client($client)->set( 'scblockgenre_list',
                $params->{'scblockgenre_list'} );
            $saved = 1;
        }
        ...
        my $replacedempty  = 0;
        my $replacekickoff = 0;
        if ( $params->{'forcenewmix'} ) {

            # Henk, 2026-09-05 - "Start Mix" (the button shown when nothing is queued at all)
            # must behave exactly like Live View's "New Mix": clear the whole queue and ask
            # MusicIP for a fresh one (AutoStartMix - same sub scStartMix calls), not swap just
            # the kick-off track while leaving anything else queued in place, which is what
            # forcereplacekickoff/ReplaceKickoffTrack below does for "Replace Again". Always the
            # "Starting..." message - the queue is always wiped here, so there is nothing to
            # call a "replace".
            AutoStartMix($client);
            $replace       = 1;
            $replacedempty = 1;
        }
        elsif ( $params->{'forcereplacekickoff'} ) {
        ...
        $params->{'prefs'}->{'scblockgenre_list'} =
          $prefs->client($client)->get('scblockgenre_list');
...
sub handleWebList {
    ...
    my $aprefs      = preferences('server');
    # Henk, 2026-09-04 - was borrowing LMS's own Classic-skin "refreshRate" server pref (floored
    # against 0/unset to avoid a tight loop). Turned out that pref is set to a real value on this
    # server - 30 (seconds) - for the Classic skin's own unrelated auto-refresh feature, which the
    # floor correctly left untouched since 30 is not "unset". Live View was consequently only
    # polling once every 30 seconds this whole time: the New Mix / Replace Track buttons' apparent
    # "sometimes 10, sometimes way more" wait was just how close a click landed to the next tick
    # of that 30-second interval, after the few seconds of staggered catch-up retries ran out.
    # Live View has no real reason to share Classic's setting at all - it's Material-only in
    # practice - so this is now a fixed, short interval of its own instead of a borrowed one.
    $params->{'refreshRate'} = 2000;

    # Auto-refresh Live View regardless of how you arrive, Henk 2026-09-04. Previously 'refresh'
    # only ever got set to 1 by quickplay.html's own redirect URL, so a player landing on Live
    # View straight from the SugarCube menu never auto-refreshed at all - the playing/up-next
    # info and recommendations just sat there stale until you reloaded by hand. Always on now;
    ...
    if ($client) {
        ...
        # Henk, 2026-09-05 - same "don't offer what cannot work" principle now applied to New
        # Mix/Replace Track too, not just the MusicIP Track Recommendations icon: SugarCube only
        # ever safely disturbs a queue of its own making, which continuous play keeps at exactly
        # one track ahead of the one playing now (CheckPosition == 2). A longer queue - a fired
        # SC Batch, a loaded album/playlist, anything with more still queued - means both buttons
        # would be replacing or clearing something that isn't SugarCube's to touch, so they're
        # hidden outright in liveview.html (sc_can_act) rather than offered and left to surprise.
        # Live View's manual controls have no business appearing once the queue is full.
        $params->{'sc_can_act'} =
...
"#### Warning $name is a slave in sync group, make changes at $sync_master ####";
        ...
        if ( !defined $checklive || $checklive == 0 ) {
            # refreshRate already set above (fixed 2s) - this used to recompute it here from
            # Classic's own pref, which would undo that. Removed 2026-09-04 (Henk).
...
sub CheckSong {
    ...
    my $sugarcube_fade_time =
      $prefs->client($client)->get('sugarcube_fade_time') || 5;   # Fade timeline
...
sub Volume_Save {
    ...
    if ( !exists $global_slide_on{$client} )
