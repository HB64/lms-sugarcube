...
### Added
- Live View & Quick Settings: a Queue button was added to the fixed player transport bar (bottom-right), opening a panel that shows the current playback queue (cover, title, artist, album/year, duration, star rating) with a remove button per track.
- Live View & Quick Settings: a thin track-progress bar now sits above the transport bar, showing playback position within the current track (visual only, not clickable/seekable).
- Live View: the "Last MIP Response" heading, Currently Playing/Coming Up Next and their action buttons now stay pinned to the top of the page while scrolling, so the MusicIP Track Recommendations list below can be scrolled independently instead of pushing everything else off-screen.

### Changed
- Live View: Currently Playing/Coming Up Next switched from a table/rowspan layout to the same flex row/column layout already used elsewhere (cover beside a stacked text column), with the existing field order, buttons and guards unchanged.
- Quick Settings: Currently Playing/Coming Up Next switched from a table-based layout to the same flex layout, fixing a cover-image alignment mismatch against Live View along the way (caused by this page's own horizontal padding, which Live View doesn't have).
- Live View: on narrow (mobile) screens, the Rating/Playcount/Last Played lines and the "Spicefly SugarCube Statistics" rows are hidden and spacing around dividers/headings is tightened, so more of the Recommendations list fits on screen without changing the desktop layout.

### Fixed
- Live View: scrolling while the MusicIP Track Recommendations list was showing could cause its text to visually overlap the pinned header above it (seen in both Chrome and Firefox) - caused by a child element's margin collapsing through the sticky header's own boundary; the header now establishes its own layout context so that can't happen.
