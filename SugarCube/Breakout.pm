...
sub getRandom {
    ...
    # Block Artist(s) (Henk, 2026-09-03) - was three separate exact-name dropdown fields
    # (scblockartist_always/alwaystwo/alwaysthree), now one comma-separated free-text pref
    # (scblockartist_list), matching saysaar's SC-EXTMIP build. Split, trim, drop empties, then
    # NOT LIKE per name so the list isn't capped at three.
    my $scblockartist_list_raw =
      $prefs->client($client)->get('scblockartist_list') || '';
    my @scblockartists =
      grep { length($_) }
      map  { s/^\s+|\s+$//g; $_ }
      split( /,/, $scblockartist_list_raw );

    $genre = $dbh->quote($genre);
    my $artistBlockClause = '';
    if (@scblockartists) {
        $artistBlockClause = join( ' AND ',
            map { "contributors.name NOT LIKE " . $dbh->quote( '%' . $_ . '%' ) }
              @scblockartists );
        $artistBlockClause = " AND $artistBlockClause";
    }
    $log->debug( @scblockartists
        ? "getRandom: blocking artist(s) [" . join( ', ', @scblockartists ) . "]\n"
        : "getRandom: no artists blocked\n" );

    my $sql =
"SELECT tracks.url, contributors.name, tracks.title, albums.title, genres.name, tracks.coverid, tracks.album FROM albums INNER JOIN contributors ON (albums.contributor = contributors.id) INNER JOIN tracks ON (tracks.album = albums.id) INNER JOIN genre_track ON tracks.id = genre_track.track INNER JOIN genres ON genre_track.genre = genres.id WHERE genres.name = $genre$artistBlockClause order by random() ASC limit 1";
...
sub getRealRandom {
    ...
    # Even the last-resort genre-blind fallback should still respect
    # genres the user has explicitly blocked - otherwise a strict genre
    # restriction can be silently ignored whenever the primary MIP-based
    # pool comes up empty for that genre.
    my $sql;
    if ($client) {
        ...
        if (@scblockgenres) {
            # LIKE-based (not exact IN) as of 2026-09-03 (Henk) - scblockgenre_list is now a free
            # text comma field (was checkbox-driven exact genre names), matching saysaar's build.
            my $genreExcludeClause = join( ' AND ',
                map { "genres.name NOT LIKE " . $dbh->quote( '%' . $_ . '%' ) }
                  @scblockgenres );
            $sql =
"SELECT tracks.url FROM tracks INNER JOIN genre_track ON tracks.id = genre_track.track INNER JOIN genres ON genre_track.genre = genres.id WHERE $genreExcludeClause order by random() limit 1";
        }
    }
    $sql ||= "SELECT tracks.url FROM tracks order by random() limit 1";
...
# Select Tracks ...Short 1 (Block Intro Tracks) tracks.secs >= 60 AND tracks.secs <= 300
  ...
  # Select Tracks ...Long 3 (>5mins) tracks.secs >= 300 AND tracks.secs <= 18000
    ...
    # sugarmipsize (Henk, 2026-09-03) - per-player "MusicIP Request No. of Tracks", moved from a
    # global-only setting so it now also governs how many tracks FreeStyle mode pulls, not just
    # Standard MusicIP mixes. Same lazy-default pattern buildMIPReq uses in Plugin.pm.
    ...
    $sugarmipsize = int($sugarmipsize) || 10;    # LIMIT needs a bare integer, not a quoted string
    ...
    if ( $sugarcube_filetype == 0 ) {   # Anything normal no file type filtering
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 1 ) {    # mp3
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high AND tracks.content_type = 'mp3' order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 2 ) {    # flac
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high AND tracks.content_type = 'flac' order by random() limit $sugarmipsize";
    }
...
# Select Tracks ...Short 1 (Block Intro Tracks) tracks.secs >= 60 AND tracks.secs <= 300
 ...
 # Select Tracks ...Long 3  (>5mins) tracks.secs >= 300 AND tracks.secs <= 18000
    ...
    if ( $sugarcube_filetype == 0 ) {   # Anything normal no file type filtering
        $sql =
"SELECT tracks.url FROM tracks WHERE (tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear) OR tracks.year = '' AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 1 ) {    # mp3
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear OR tracks.year = '' AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high AND tracks.content_type = 'mp3' order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 2 ) {    # flac
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.year >= $sugarcube_startyear AND tracks.year <= $sugarcube_endyear OR tracks.year = '' AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high AND tracks.content_type = 'flac' order by random() limit $sugarmipsize";
    }
...
# Select Tracks ...Short 1 (Block Intro Tracks) tracks.secs >= 60 AND tracks.secs <= 300
 ...
 # Select Tracks ...Long 3  (>5mins) tracks.secs >= 300 AND tracks.secs <= 18000
    ...
    if ( $sugarcube_filetype == 0 ) {   # Anything normal no file type filtering
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 1 ) {    # mp3
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.content_type = 'mp3' AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high order by random() limit $sugarmipsize";
    }
    elsif ( $sugarcube_filetype == 2 ) {    # flac
        $sql =
"SELECT tracks.url FROM tracks WHERE tracks.content_type = 'flac' AND tracks.secs >= $sugarcube_fs_length_low AND tracks.secs <= $sugarcube_fs_length_high order by random() limit $sugarmipsize";
    }
...
"CREATE TABLE IF NOT EXISTS WorkingSet (id INTEGER PRIMARY KEY, client, trackingno, temptrack, SCtrack, SCalbum, SCgenres, SCartist, SCplaycount integer, SCrating integer, SClastplayed integer, cover, album)"
    ...
    $dbh->do(
"CREATE TABLE IF NOT EXISTS AlbumTracker (id INTEGER PRIMARY KEY, client, SCalbum, UNIQUE(client, SCalbum))"
    );
    $dbh->do(
"CREATE TABLE IF NOT EXISTS ArtistTracker (id INTEGER PRIMARY KEY, client, SCartist, UNIQUE(client, SCartist))"
    );
    $dbh->do(
"CREATE TABLE IF NOT EXISTS TrackTracker (id INTEGER PRIMARY KEY, client, track, UNIQUE(client, track))"
    );
    $dbh->do(
"CREATE TABLE IF NOT EXISTS History (id INTEGER PRIMARY KEY, client, artist,track,album,genre,albumart,fullalbum)"
      ...
      )
    {
        ...
        my $col   = $trackerdef->[1];
        my ($tablesql) = $dbh->selectrow_array(
        ...
        if ( $tablesql && $tablesql !~ /unique\s*\(\s*client\s*,\s*$col\s*\)/i ) {
            ...
            $dbh->do(
"CREATE TABLE $table (id INTEGER PRIMARY KEY, client, $col, UNIQUE(client, $col))"
            );
            $dbh->do(
...
###
# applyArtistWeighting - rewritten 2026-09-03 (Henk) to the comma-list convention, matching
# saysaar's SC-EXTMIP build: ONE list of Preferred names sharing ONE weight (1-5), and ONE list
# of Less Preferred names sharing ONE weight (1-5) - was three individually-weighted name slots
# per side. SOFT bias, not a hard exclusion: a Preferred match duplicates a track (weight+1)
# times in the array before random selection, a Less Preferred match randomly drops the track
# with probability 1 - 1/(weight+1). If every track ends up dropped, the original set is
# returned unchanged rather than handing the caller an empty pool.
###
            ...
            if ( $sth->fetch() ) {
                if ( defined($songIndex) ) {
                    ...
                    {    #  sugarcube_blockartist
                        ...
                        {
                            my $sth = $dbh->prepare(
"delete from AlbumTracker where id in (select id from AlbumTracker where client='$clientid' order by id asc limit 1)"
                            );
                            $sth->execute();
                        }
                    }
        ...
        if ( $sth->fetch() ) {
            if ( defined($songIndex) ) {
                ...
                {    #  sugarcube_blockartist
                    ...
                    {
                        my $sth = $dbh->prepare(
"delete from ArtistTracker where id in (select id from ArtistTracker where client='$clientid' order by id asc limit 1)"
                        );
                        $sth->execute();
                    }
                }
        ...
        if ( $sth->fetch() ) {
            if ( defined($songIndex) ) {
                ...
                {    #  sugarcube_remembertracks
                    ...
                    {
                        my $sth = $dbh->prepare(
"delete from TrackTracker where id in (select id from TrackTracker where client='$clientid' order by id asc limit 1)"
                        );
                        $sth->execute();
                    }
                }
...
sub mystuff {
    ...
    my $sth = $dbh->prepare(
"SELECT temptrack, SCTrack, SCalbum, SCgenres, SCartist, SCplaycount, SCrating, SClastplayed, cover, album FROM WorkingSet WHERE WorkingSet.trackingno = 'OK' AND client ='$clientid' ORDER BY id ASC"
    );
    $sth->execute();
...
sub DropGenreAndXMas {
    ...
    my @scblockgenres =
      ...
      map  { s/^\s+|\s+$//g; $_ }
      split( /,/, $scblockgenre_list_raw );
    # LIKE-based (not exact IN) as of 2026-09-03 (Henk) - scblockgenre_list is now a free text
    # comma field (was checkbox-driven exact genre names), matching saysaar's build: a partial
    # name is enough to match.
    my $genreBlockClause;
    if (@scblockgenres) {
        $genreBlockClause = join( ' OR ',
            map { "WorkingSet.SCgenres LIKE " . $dbh->quote( '%' . $_ . '%' ) }
              @scblockgenres );
    }
    else {
        $genreBlockClause = '1=0';
    }
    my $clientid = $dbh->quote( Slim::Player::Client::id($client) );

    $log->debug( @scblockgenres
        ? "DropGenreAndXMas: blocking genres [" . join( ', ', @scblockgenres ) . "]\n"
        : "DropGenreAndXMas: no genres blocked\n" );
    ...
    if ( ( $sugarxmas == 1 ) && ( $theTime ne 'Dec' ) ) {
        ...
        $sth = $dbh->prepare(
"UPDATE WorkingSet SET trackingno = 'DROPBLOCKEDGENRE' WHERE ($genreBlockClause OR upper(WorkingSet.SCgenres) = 'XMAS' OR upper(WorkingSet.SCgenres) = 'CHRISTMAS') AND WorkingSet.client = $clientid"
        );
        $sth->execute() || die "Could not execute: " . $dbh->errstr;
        $log->debug( "DropGenreAndXMas: marked " . $sth->rows . " WorkingSet row(s) as DROPBLOCKEDGENRE (incl. Xmas check)\n" );
    }
    else {
        $sth = $dbh->prepare(
"UPDATE WorkingSet SET trackingno = 'DROPBLOCKEDGENRE' WHERE ($genreBlockClause) AND WorkingSet.client = $clientid"
        );
        $sth->execute() || die "Could not execute: " . $dbh->errstr;
        $log->debug( "DropGenreAndXMas: marked " . $sth->rows . " WorkingSet row(s) as DROPBLOCKEDGENRE\n" );
    }
...
sub DropArtists {
    ...
    # Block Artist(s) we dont want - comma-separated free text as of 2026-09-03 (Henk), matching
    # saysaar's build (was three exact-name dropdown fields, capped at three). LIKE-matched so a
    # partial name is enough; blank/empty pieces skipped so an empty list blocks nothing.
    my $scblockartist_list_raw =
      $prefs->client($client)->get('scblockartist_list') || '';
    my @scblockartists =
      grep { length($_) }
    ...
    my $artistBlockClause;
    if (@scblockartists) {
        $artistBlockClause = join( ' OR ',
            map { "WorkingSet.SCartist LIKE " . $dbh->quote( '%' . $_ . '%' ) }
              @scblockartists );
    }
    else {
        $artistBlockClause = '1=0';
    }
    $log->debug( @scblockartists
        ? "DropArtists: blocking artist(s) [" . join( ', ', @scblockartists ) . "]\n"
        : "DropArtists: no artists blocked\n" );
    ...
    if ( $sugarcube_global_player == 0 ) {
        ...
        my $clientid = $dbh->quote( Slim::Player::Client::id($client) );
        $sth = $dbh->prepare(
"UPDATE WorkingSet SET trackingno = 'DROPBLOCKEDARTIST' WHERE ($artistBlockClause) AND WorkingSet.client = $clientid"
        );
        $sth->execute() || die "Could not execute: " . $dbh->errstr;
        $sth = $dbh->prepare(
...
sub DropEmPunk {
    ...
    my $sth = $dbh->prepare(
"SELECT DISTINCT WorkingSet.id FROM WorkingSet INNER JOIN TrackTracker ON TrackTracker.client = WorkingSet.client WHERE WorkingSet.temptrack = TrackTracker.track AND WorkingSet.client ='$clientid'"
    );
    $sth->execute();
    my $array_ref = $sth->fetchall_arrayref();
    foreach my $row (@$array_ref) {
...
"SELECT trackingno, SCtrack, SCalbum, SCartist, SCgenres, SCplaycount, SCrating, SClastplayed, temptrack, cover, album, trackid FROM WorkingSet WHERE WorkingSet.client = '$clientid' ORDER BY WorkingSet.trackingno DESC"
    ...
    # Henk, 2026-09-05 - "don't offer what cannot work", ported from the saysaar build's Live
    # View "Use as Next Track": SugarCube only ever disturbs a track IT queued itself, which
    # continuous play keeps to exactly one ahead of the one now playing (CheckPosition == 2).
    # More than that means a fired SC Batch or a queue Henk built by hand, and swapping one
    # track out of it would just make the queue shorter at an arbitrary point, not a genuine
    # "play next" - so the icon itself is simply not drawn for the whole page in that case,
    # computed once here rather than re-checked per row. scPlayNextTrack (Plugin.pm) keeps the
    # same guard server-side too, since a batch can still fire in the seconds between drawing
    # this page and clicking - hiding the icon is tidiness, the handler's own guard is safety.
    my $scCanReplace = ( CheckPosition($client) <= 2 ) ? 1 : 0;
    ...
    if ($sugarlvTS) {
        ...
        while ( $sth->fetch ) {
            ...
            # Henk, 2026-09-05 - was 4 separate icons (Play Album/Add Album/Add
            # Track to Queue/More-browse), inspired by the saysaar build: down to
            # one icon that swaps this track into the next-to-play slot (see
            # scPlayNextTrack in Plugin.pm - delete the current "next" track,
            # then playlist-insert this one, so the queue length doesn't grow).
            # Icon is a combined "play next + queue" glyph Henk supplied
            # (cs-queue.svg + queue_music_outline.svg, merged): a play triangle
            # and vertical bar (play-next), with the queue's three stripes
            # immediately to the right of that bar instead of Material's own
            # second vertical bar. fill="currentColor" plus the .sc-mipbtn class
            # (liveview.html) follows --text-color the same way as the New
            # Mix/Replace Track icons - see the CSS notes there.
            # Not drawn at all when $scCanReplace is false - see the comment
            # above the while loop.
            my $buildnext = '&nbsp;';
            if ($scCanReplace) {
                $buildnext =
"<a class=\"sc-mipbtn\" onclick=\"var r=new XMLHttpRequest();r.open('POST','/jsonrpc.js',true);r.setRequestHeader('Content-Type','application/json');r.onload=function(){};r.send(JSON.stringify({id:1,method:'slim.request',params:['"
                  . $clientid_raw
                  . "',['sugarcube','playnext','"
                  . $col12
                  . "']]}));\" title=\"Play Next\" aria-label=\"Play Next\">"
                  . '<svg width="40" height="24" viewBox="0 0 40 24" fill="currentColor"><polygon points="4,4 4,20 14,12"/><rect x="16" y="4" width="2.4" height="16"/><rect x="21" y="5" width="16" height="2.4"/><rect x="21" y="11" width="16" height="2.4"/><rect x="21" y="17" width="11" height="2.4"/></svg></a>';
            }

            # THIS IS STATISTICS BUILD UP
            $line =
                $line
              . '<tr><td rowspan=9 class=pic><img style="border-radius:10px !important; object-fit:cover !important; display:block !important;" width='
              . $sugarlviconsize
              . ' height='
              . $sugarlviconsize
              . " src=/music/"
        ...
        while ( $sth->fetch ) {
            ...
            # Henk, 2026-09-05 - see the matching comment in the stats-enabled branch above and
            # the one above the while loop: one "Play Next" icon (scPlayNextTrack) instead of the
            # previous four, not drawn at all when $scCanReplace is false (batch/hand-built queue).
            my $buildnext = '&nbsp;';
            if ($scCanReplace) {
                $buildnext =
"<a class=\"sc-mipbtn\" onclick=\"var r=new XMLHttpRequest();r.open('POST','/jsonrpc.js',true);r.setRequestHeader('Content-Type','application/json');r.onload=function(){};r.send(JSON.stringify({id:1,method:'slim.request',params:['"
            ...
            $line =
                $line
              . '<tr><td rowspan=7 class=pic><img style="border-radius:10px !important; object-fit:cover !important; display:block !important;" width='
              . $sugarlviconsize
              . ' height='
              . $sugarlviconsize
              . " src=/music/"
            ...
            if ( defined($songIndex) ) {
                if ( $songIndex > 30 ) {
                    for ( my $i = 0 ; $i < $songIndex - 30 ; $i++ ) {
                        my $sth = $dbh->prepare(
"delete from History where id in (select id from History where client='$clientid' order by id asc limit 1)"
                        );
                        $sth->execute();
                    }
                }
